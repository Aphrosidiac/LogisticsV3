...
export async function getCompletedOrders(): Promise<CompletedOrder[]> {
  try {
    ...
    return (data || []).map((order: Record<string, unknown>) => ({
      ...
      measurement_unit: (order.measurement_unit as Order['measurement_unit']) || 'CTN',
      is_oversized: (order.is_oversized as boolean) || false,
      pallets_max: (order.pallets_max as number) || undefined,
      ...
      delivery_photo_urls: (order.delivery_photo_urls || []) as string[],
      delivered_at: (order.delivered_at || undefined) as string | undefined,
